...
def Qwen_chat(tokenizer, query, history, stop_words_ids, **gen_kwargs):
    conversation = [
...
class SpeechRequest(BaseModel):
    model: str
    input: str
    voice: str = "alloy"  # man 7869 2222 6653 women 4099 5099
...
def text_complete_last_message(history, stop_words_ids, gen_kwargs):
    ...
    output = model.generate(
        input_ids, stop_words_ids=stop_words_ids, **gen_kwargs
    ).tolist()[0]
...
async def get_embeddings(request: EmbeddingRequest):
    ...
    if isinstance(request.input, str):
        texts = [request.input]
    elif isinstance(request.input[0], list):  # 检查输入类型
        # 处理 List[List[int]]
        encoding = tiktoken.encoding_for_model(request.model)
        texts = [encoding.decode(text) for text in request.input]
...
async def create_speech(request: SpeechRequest):
    ...
    try:
        ...
        print(f"[tts]{input_text=}\n{voice=},{speed=}\n")
        wavs = generate_speech(
            input_text, voice, temperature=temperature, prompt=prompt
        )
        ...
        sf.write(wav_file_path, wavs[0][0], samplerate=24000)
        return FileResponse(
            wav_file_path, media_type=f"audio/{request.response_format}"
        )
...
def _get_args():
    ...
    parser.add_argument(
        "--server-port", type=int, default=4073, help="Demo server port."
    )
    parser.add_argument(
        "--server-name",
        type=str,
...
if __name__ == "__main__":
    ...
    model.chat = Qwen_chat
    embeddings_model = SentenceTransformer(DEFAULT_EMBEDDING_PATH, device="cpu")
